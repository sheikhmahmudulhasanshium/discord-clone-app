...
const ServerChannel = ({channel,server,role}:ServerChannelProps) => {
    ...
    >
        ...
        <p className={cn(
            "line-clamp-1 text-sm font-semibold text-zinc-500 group-hover:text-zinc-800 dark:group-hover:text-zinc-200 transition",
        ...
        {channel.name!=="general"&& role!==MemberRole.GUEST && (
            <div className="flex items-center ml-auto gap-x-2">
                ...
                <ActionTooltip label={"Delete"}>
                    <Trash className="hidden w-4 h-4 transition group-hover:block text-zinc-500 hover:text-zinc-600 dark:text-zinc-400 dark:hover:text-zinc-100"/>
                </ActionTooltip>
            </div>
        )}
